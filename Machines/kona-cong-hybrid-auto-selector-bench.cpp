...
const int K = 64;
const int LAN_CALIBRATION_DCF_MAX_COMPARATORS = 160;
const int OLD_HYBRID_DCF_CANDIDATE_THRESHOLD = 1024;

int playerno = 0;
ez::ezOptionParser opt;
string backend_mode = "auto";
string size_list = "1024,2048,4096";
int topk_k = 5;
int lan_calibration_threshold = LAN_CALIBRATION_DCF_MAX_COMPARATORS;
...
struct TopkBenchResult
{
    ...
    int lan_calibration_dcf_max_comparators = LAN_CALIBRATION_DCF_MAX_COMPARATORS;
...
void parse_argv(int argc, const char** argv)
{
    ...
    opt.add("auto", 0, 1, 0,
            "Backend: pcr, dcf, old-hybrid, auto", "-m", "--mode");
    opt.add("1024,2048,4096", 0, 1, 0, "Comma-separated n list", "-n", "--sizes");
    opt.add("5", 0, 1, 0, "Top-k", "-k", "--topk");
    opt.add("160", 0, 1, 0,
            "LAN calibration threshold: use DCF when comparator_count <= this value",
    ...
    opt.get("--calibration-threshold")->getInt(lan_calibration_threshold);
    ...
    if (backend_mode != "pcr" && backend_mode != "dcf" &&
            backend_mode != "old-hybrid" && backend_mode != "auto")
        throw runtime_error("backend mode must be pcr, dcf, old-hybrid, or auto");

    if (topk_k <= 0)
        throw runtime_error("top-k must be positive");

    if (lan_calibration_threshold < 0)
        throw runtime_error("calibration threshold must be non-negative");
}
...
// Old candidate-size heuristic used by the previous hybrid benchmark.
// This remains as a baseline mode only; the auto selector below does not use it.
std::vector<int> estimate_effective_candidate_sizes(
        const KonaCongTopK::CongNetwork& network,
        int n)
{
    std::vector<int> result(network.levels.size(), 0);
...
Backend select_backend(size_t comparator_count, int calibration_threshold)
{
    // LAN calibration source: kona-layer-cost-calibration-bench.x measured
    // compare + ss_vec_kona_l2 and found DCF faster up to m=160, PCR faster
    // from m=256 for the current LAN profile. Keep this centralized so it can
    // be replaced later by a PCR/DCF cost lookup table.
    return comparator_count <= static_cast<size_t>(calibration_threshold) ?
            Backend::DCF : Backend::PCR;
}

const char* backend_name(Backend backend)
{
    return backend == Backend::DCF ? "dcf" : "pcr";
}
        ...
        int calibration_threshold)
{
    ...
    if (mode == "old-hybrid")
        effective_sizes = estimate_effective_candidate_sizes(network, n);
    ...
    for (size_t depth = 0; depth < network.levels.size(); ++depth)
    {
        ...
        if (mode == "dcf")
            ...
            backend = Backend::PCR;
        else if (mode == "old-hybrid")
            backend = effective_sizes.at(depth) <= OLD_HYBRID_DCF_CANDIDATE_THRESHOLD ?
                    Backend::DCF : Backend::PCR;
        else
            backend = select_backend(m, calibration_threshold);
...
void print_level_plan(const TopkBenchResult& r)
{
    ...
    for (const auto& p : r.plan)
    {
        cout << "CONG_LEVEL_PLAN"
             ...
             << " threshold=" << r.lan_calibration_dcf_max_comparators
             << endl;
    }
}
...
{
    cout << "CONG_TOPK_AUTO_SELECTOR"
         << " backend=" << r.backend
         << " n=" << r.n
         << " k=" << r.k
         << " calibration_threshold=" << r.lan_calibration_dcf_max_comparators
        ...
        RealTwoPartyPlayer* player)
{
    ...
    res.backend = mode == "pcr" ? "cong-pcr" :
                  mode == "dcf" ? "cong-dcf" :
                  mode == "old-hybrid" ? "cong-old-hybrid" :
                                          "cong-auto-hybrid";
    res.n = static_cast<int>(shares.size());
    res.k = k;
    res.lan_calibration_dcf_max_comparators = lan_calibration_threshold;
    ...
    res.plan = build_level_plan(network, mode, res.n, lan_calibration_threshold);
    fill_plan_stats(res);
