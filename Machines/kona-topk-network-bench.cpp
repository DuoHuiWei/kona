...
void parse_argv(int argc, const char** argv, int& n, int& k)
{
    ...
    opt.add("legacy", 0, 1, 0, "Benchmark mode: legacy, cong, cong-dcf, matrix",
            "-m", "--mode");
    opt.parse(argc, argv);
    if (opt.isSet("-p"))
    ...
    if (mode != "legacy" && mode != "cong" && mode != "cong-dcf" && mode != "matrix")
        ...
        RealTwoPartyPlayer* player)
{
    ...
    finalize_vote_and_winner_with_pcr(shares, k, player, res);
