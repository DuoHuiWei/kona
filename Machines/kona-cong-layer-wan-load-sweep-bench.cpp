...
int background_load_percent = 100;
...
void background_burn_iters(uint64_t& x, uint64_t& count, int iterations)
{
        ...
        double* cpu_ms_out = 0)
{
    ...
    while (!stop.load(memory_order_relaxed))
        background_burn_iters(x, count, 1024);
    if (cpu_ms_out)
        *cpu_ms_out = thread_cpu_ms_now() - cpu_start_ms;
    return count ^ (x & 1ULL);
}

uint64_t background_compute_duty_cycle_until(
        const atomic<bool>& stop,
        int load_percent,
        double* cpu_ms_out = 0)
{
    ...
    if (load_percent >= 100)
        return background_compute_until(stop, cpu_ms_out);

    // One-millisecond duty cycle: burn for load_percent * 10 us, then yield/sleep.
    const chrono::microseconds period(1000);
    const chrono::microseconds active_us(load_percent * 10);
    while (!stop.load(memory_order_relaxed))
    {
        auto cycle_start = chrono::steady_clock::now();
        auto active_end = cycle_start + active_us;
        while (!stop.load(memory_order_relaxed) &&
                chrono::steady_clock::now() < active_end)
        {
            background_burn_iters(x, count, 256);
        }

        auto cycle_end = cycle_start + period;
        while (!stop.load(memory_order_relaxed) &&
                chrono::steady_clock::now() < cycle_end)
        {
...
void parse_argv(int argc, const char** argv)
{
    ...
    opt.add("100", 0, 1, 0, "Background duty-cycle load percent: 0,25,50,75,100", "-load", "--background-load");
    opt.parse(argc, argv);
    ...
    opt.get("--background-load")->getInt(background_load_percent);
    ...
    if (background_load_percent < 0 || background_load_percent > 100)
        throw runtime_error("background load percent must be in [0, 100]");
}
...
{
    cout << "KONA_LAYER_WAN_LOAD_SWEEP"
         << " backend=" << r.backend
         << " comparator_count=" << r.comparator_count
         << " with_background=" << (r.with_background ? 1 : 0)
         << " background_load_percent=" << r.background_load_percent
        ...
        RealTwoPartyPlayer* player)
{
    ...
    res.background_load_percent = load_percent;
    ...
    if (enable_background)
        background_thread = thread([&]() {
            background_count = background_compute_duty_cycle_until(
                    stop_background, load_percent, &background_cpu_ms);
        });
...
int main(int argc, const char** argv)
{
    ...
    try
    {
        BenchResult result = run_layer_bench(
                backend_mode, comparator_count, with_background,
                background_load_percent, player);
        if (playerno == 0)
            print_result(result);
    }
    catch (...)
